...
const hasDirection = (dir, target) => new RegExp(dir, 'i').test(target);
...
export class Resizable extends React.PureComponent {
    ...
    onMouseMove(event) {
        ...
        const parentSize = this.getParentSize();
        const max = calculateNewMax(parentSize, this.window.innerWidth, this.window.innerHeight, maxWidth, maxHeight, minWidth, minHeight);
    ...
    renderResizer() {
        ...
        const resizers = Object.keys(enable).map(dir => {
            if (enable[dir] !== false) {
                return (_jsx(Resizer, { direction: dir, onResizeStart: this.onResizeStart, replaceStyles: handleStyles && handleStyles[dir], className: handleClasses && handleClasses[dir], children: handleComponent && handleComponent[dir] ? handleComponent[dir] : null }, dir));
            }
            return null;
        });
    ...
    render() {
        ...
        return (_jsxs(Wrapper, { ref: this.ref, style: style, className: this.props.className, ...extendsProps, children: [this.state.isResizing && _jsx("div", { style: this.state.backgroundStyle }), this.props.children, this.renderResizer()] }));
    }
}
